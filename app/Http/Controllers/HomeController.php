...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $alumnos = alumno::select('id','nombres','apellido_paterno', 'apellido_materno')->orderBy('created_at','DESC') ->take(5)->get();
        $cursos = curso::select('id','nombre','clave','created_at')->orderBy('created_at','DESC')->take(5)->get();
        $lista_cursos = curso::select('id','nombre')->where('periodo_id',\Session::get('idPeriodo'))->orderBy('nombre','ASC')->get();
        $listaRangos = periodos_rangos::select('id','nombre')->where('periodo_id',\Session::get('idPeriodo'))->get();
        return view('home', compact('results', 'alumnos', 'cursos', 'lista_cursos', 'listaRangos', 'users', 'subcomponentes', 'series', 'notas', 'ns'));
    }
